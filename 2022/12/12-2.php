 ...
 * which is applied to 2dimensional grid and finds the shortest path from Start to End blazingly fast.
 */
class Coord
{
...
class Grid
{
    ...
    private array $usedStartingPoints = [];
    ...
    public function __construct(
        ...
        $this->usedStartingPoints[$this->start->x.':'.$this->start->y] = true;
    }

    public function resetAndChangeStartingPoint(): bool
    {
        // Reset visited array
        $this->visited = [];

        // Find new starting pos for the grid
        for($y = 0; $y < $this->nRows; $y++) {
            for($x = 0; $x < $this->nCols; $x++) {
                if ($this->grid[$y][$x] === 'a') {
                    $coord = new Coord($x, $y, 0);

                    if (!isset($this->usedStartingPoints[$coord->x.':'.$coord->y])) {
                        $this->start = $coord;
                        $this->usedStartingPoints[$this->start->x.':'.$this->start->y] = true;
                        return true;
                    }
...
do {
    $finish = find_shortest_distance_to_finish($grid);

    if ($finish) {
        if ($minDist === null || $finish->dist < $minDist['finish']->dist) {
            $minDist = [
                'start' => $grid->getStart(),
                'finish' => $finish
            ];
...
} while($grid->resetAndChangeStartingPoint());

if ($minDist === null) {
    throw new Exception("There is not path from Start to Finish");
} else {
    echo "Found shortest path from any cell with elevation 'a' to E:\n";
    echo "  start:  x={$minDist['start']->x},y={$minDist['start']->y}\n";
    echo "  finish: x={$minDist['finish']->x},y={$minDist['finish']->y}\n";
    echo "  distance: {$minDist['finish']->dist}\n";
}